...
def has_phenotypes(phenotypes, article_hpo_terms):
    article_hpo_ids = [
        article_hpo_term["key"] for article_hpo_term in article_hpo_terms
    ]
    intersection = [
        hpo_id for hpo_id in phenotypes.values() if hpo_id in article_hpo_ids
    ]
...
def aggregate_article_info(variant_info, phenotypes):
    ...
    for variant, values in variant_info.items():
        ...
        for pmid in values["pmids"]:
            ...
            print_progress(
                current,
                total,
                prefix="Inspecting PMID info for " + str(variant) + ":",
                suffix="Complete",
                bar_length=50,
            )
            ...
            if FILTER_ON_PHENOTYPES and (
                "hpo_terms" not in data
                or not has_phenotypes(phenotypes, data["hpo_terms"])
            ):
            ...
            for pmid_gene in data["genes"]:
                ...
                if "variants" in pmid_gene:
                    for pmid_variant in pmid_gene["variants"]:
                        pmids_by_variant[variant_key(pmid_gene, pmid_variant)].append(
                            pmid
                        )
            ...
            if "hpo_terms" in data:
                for phenotype in data["hpo_terms"]:
                    ...
                    if phenotype["term"] in phenotypes.keys():
                        ...
                        if "diseases" in data:
                            phenotype_info[phenotype["term"]]["diseases"].update(
                                pmid_diseases
                            )
                        ...
                        phenotype_info[phenotype["term"]]["variants"].update(
                            pmid_variants
                        )

            pmid_comentioned_variants = [
                input_variant
                for input_variant in variant_info.keys()
                if input_variant.upper()
                in [pmid_variant.upper() for pmid_variant in pmid_variants]
            ]
            pmid_comentioned_variants.sort()
            pmid_comentioned_phenotypes = [
                input_phenotype_term
                for input_phenotype_term in phenotypes.keys()
                if input_phenotype_term in pmid_phenotypes
            ]
            ...
            if len(pmid_comentioned_variants) > 1:
                ...
                comentioned_variants[comentioned_key]["phenotypes"].update(
                    pmid_comentioned_phenotypes
                )
            ...
            if len(pmid_comentioned_phenotypes) > 1:
                ...
                comentioned_phenotypes[comentioned_key]["variants"].update(
                    pmid_comentioned_variants
                )
                comentioned_phenotypes[comentioned_key]["diseases"].update(
                    pmid_diseases
                )
    ...
    return (
        ...
        filtered_article_info,
        phenotype_info,
        comentioned_variants,
        comentioned_phenotypes,
    )
...
def main(args):
        ...
        for line in lines:
            ...
            if len(variant_data) > 0:
                ...
                if len(gene_data) > 0:
                    ...
                    variant_data = api_get(
                        "suggestions", {"variant": canonical_gene + ":" + gene_input[1]}
                    )
                    if len(variant_data) > 0:
                        canonical_variant = variant_data[0]["canonical"]
                    else:
    ...
    (
        ...
        comentioned_variants,
        comentioned_phenotypes,
    ) = aggregate_article_info(variant_info, phenotypes)
    ...
    with codecs.open(articles_file_path, "wb", "utf-8") as output_file:
        ...
        for pmid, article in article_info.items():
            ...
            for gene in article["genes"]:
                ...
                if "variants" in gene:
                    pmid_variants.extend(
                        [
                            gene["symbol"] + ":" + variant["key"]
    ...
    with codecs.open(variants_file_path, "wb", "utf-8") as output_file:
        ...
        for variant, info in variant_info.items():
            variant_diseases = [
                variant_disease for variant_disease in info["diseases"].keys()
            ]
            variant_phenotypes = [
                variant_phenotype for variant_phenotype in info["phenotypes"].keys()
            ]
            variant_genes = [variant_gene for variant_gene in info["genes"].keys()]
            variant_variants = [
                variant_variant for variant_variant in info["variants"].keys()
            ]
            output_file.write(
                ",".join(
                    [
    ...
    with codecs.open(associations_summary_file_path, "wb", "utf-8") as output_file:
        ...
        if len(comentioned_variants) > 0:
            ...
            for variants, data in sorted(
                comentioned_variants.items(),
                key=lambda item: 1
                if len(item[1]["phenotypes"]) == 0
                else 1 / len(item[1]["phenotypes"]),
            ):
        ...
        if len(comentioned_phenotypes) > 0:
            ...
            for phenotypes, data in sorted(
                comentioned_phenotypes.items(),
                key=lambda item: 1 / len(item[1]["variants"]),
            ):
    ...
    with codecs.open(associations_file_path, "wb", "utf-8") as output_file:
        ...
        for variant, data in variant_info.items():
            if len(data["pmids"]) == 0:
                ...
                output_file.write(
                    "Found the following associations for " + str(variant) + ":\n"
                )
                ...
                if len(data["genes"]) == 0:
                    ...
                    for values in sorted(
                        ...
                        if (
                            OMIT_ONE_PMID_MATCHES_FROM_ASSOCIATIONS_FILE
                            and len(values[1]) <= 1
                        ):
                            output_file.write(
                                "\t\t(Truncated associations with only one PMID)\n"
                            )
                            break
                        output_file.write(
