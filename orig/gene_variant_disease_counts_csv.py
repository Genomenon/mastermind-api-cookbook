...
def generate_lines(variant, canonical_disease):
    ...
    for match in variant_data:
        ...
        variant_disease_count_data = api_get(
            "counts", {"variant": canonical_variant, "disease": canonical_disease}
        )
        ...
        if "diseases" in variant_disease_data:
            for disease in variant_disease_data["diseases"]:
                variant_diseases_with_counts.append(
                    str(disease["key"]) + "(" + str(disease["article_count"]) + ")"
                )
        ...
        gene_disease_count_data = api_get(
            "counts", {"gene": canonical_gene, "disease": canonical_disease}
        )
        ...
        if "diseases" in gene_disease_data:
            for disease in gene_disease_data["diseases"]:
                gene_diseases_with_counts.append(
                    str(disease["key"]) + "(" + str(disease["article_count"]) + ")"
                )
...
def main(args):
    ...
    output.append(
        [
            ...
            "Variant "
            + '"'
            + re.sub(r"\"", '""', canonical_disease)
            + '"'
            + " Articles in MM (Article Count",
            "Variant Article Count in MM",
            "Variant Diseases in MM (Article Count)",
            "MM Gene Link",
            "Gene " + canonical_disease + " Articles in MM (Article Count)",
            "Gene Article Count in MM",
            "Gene Diseases in MM (Article Count)",
        ]
    )
